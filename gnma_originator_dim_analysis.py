...
FROM (
SELECT DISTINCT
	UPPER(TRIM(LOAN_ORIGINATION_COMPANY)) AS SOURCE_ORIGINATOR
FROM [DW_LOAN_VALUATION].[stage].[HECM_DIRTY_LOANS] WITH(NOLOCK)
WHERE
	NULLIF(LOAN_ORIGINATION_COMPANY, '') IS NOT NULL
) A
...
out_file = r'c:\temp\gnma_originator_dim_analysis.csv'
